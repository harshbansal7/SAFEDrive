...
LANDMARK_PREDICTOR = dlib.shape_predictor("model_full_mouth_jmd.dat")
    ...
    if len(faces) > 0:
        for k, face in enumerate(faces):
            ...
            if ear > 0.27:
                ...
                if total > 20:
                    if config['SCREEN_PRINTS']:
                        cv2.putText(frame, f"Drowsiness Detected", (250, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                
                if config['SCREEN_PRINTS']:
                    cv2.putText(frame, f"Eyes Closed | EAR = {round(ear, 2)}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
